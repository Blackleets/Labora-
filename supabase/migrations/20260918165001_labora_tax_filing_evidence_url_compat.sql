...
-- Keeps the current Labora+ UI compatible while retaining explicit filing evidence.

alter table public.tax_declarations
  add column if not exists filing_evidence_url text;
...
begin
  if tg_op = 'INSERT' then
    if new.status <> 'draft' then raise exception 'Tax declaration must start as draft'; end if;
    new.reviewed_by := null;
    new.reviewed_at := null;
    new.review_note := null;
    new.filing_reference := null;
    new.filing_evidence_url := null;
    new.filed_at := null;
    new.filed_by := null;
  ...
  if old.status = new.status then
    ...
    if old.reviewed_by is distinct from new.reviewed_by
       ...
       or old.filing_evidence_url is distinct from new.filing_evidence_url
  ...
  if old.status = 'reviewed_by_gestor' and new.status = 'filed_with_tax_agency' then
    ...
    if nullif(btrim(coalesce(new.filing_reference, '')), '') is null
       and nullif(btrim(coalesce(new.filing_evidence_url, '')), '') is null
       and new.filing_document_id is null then
      raise exception 'Filing reference or evidence is required';
    end if;
...
create or replace function private.file_tax_declaration_url_internal(
...
begin
  ...
  if nullif(btrim(coalesce(p_filing_reference,'')), '') is null
     and nullif(btrim(coalesce(p_evidence_url,'')), '') is null then
    raise exception 'Filing reference or evidence is required';
  end if;
  ...
  set status = 'filed_with_tax_agency',
      ...
      filing_evidence_url = nullif(btrim(coalesce(p_evidence_url,'')), ''),
      filed_at = current_date,
      updated_at = now()
...
revoke all on function private.file_tax_declaration_url_internal(text,text,text) from public, anon;
grant execute on function private.file_tax_declaration_url_internal(text,text,text) to authenticated;
...
set search_path = public, private
as $$ select private.file_tax_declaration_url_internal(p_declaration_id,p_filing_reference,p_evidence_url); $$;

revoke all on function public.file_tax_declaration(text,text,text) from public, anon;
grant execute on function public.file_tax_declaration(text,text,text) to authenticated;
