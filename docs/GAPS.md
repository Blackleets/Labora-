...
| Data integrity release gate | Snapshot sync no longer infers deletes from absent local rows; liquidación + ingresos vinculados use the rider-only transactional RPC `delete_own_document_with_linked_incomes`; persistent tombstones plus advisory locks reject stale recreation from another tab/device; migrations `20260921170000`, `20260921173000`, `20260921180000`, `20260921183000` and `20260921184000` are applied on Supabase |
