...
import { Play, CheckCircle, XCircle, Terminal, RefreshCw, Activity, CreditCard } from 'lucide-react';
import { useCountry } from '../../contexts/CountryContext';
import { pricingEngine, PricingRequest } from '../../modules/country-config/services/pricingEngine';
import { MockBankAdapter } from '../../modules/banking-connect/services/bankAdapter';
...
export const TestRunner: React.FC = () => {
  ...
  const [bankTests, setBankTests] = useState<TestResult[]>([
    { id: 'b1', name: 'Connect Bank (Mock)', status: 'pending', log: 'Auth flow simulation' },
    { id: 'b2', name: 'Fetch Accounts', status: 'pending', log: 'Get balance & details' },
    { id: 'b3', name: 'Simulate Payout Transfer', status: 'pending', log: 'Send 50 EUR to Main Account' },
  ]);
  ...
  const runBankTest = async (test: TestResult): Promise<TestResult> => {
    ...
    try {
      if (test.id === 'b1') {
        const { authUrl } = await adapter.initConnection('test_user');
        if (!authUrl.includes('mock-auth')) throw new Error("Invalid Auth URL");
        await adapter.completeConnection({ code: 'test' });
      }
      if (test.id === 'b2') {
        const accounts = await adapter.getAccounts('conn_test');
        if (accounts.length === 0) throw new Error("No accounts found");
      }
      if (test.id === 'b3') {
        const tx = await adapter.createTransfer({ fromAccountId: 'acc_1', amount: 50, currency: 'EUR' });
        if (tx.status !== 'completed') throw new Error("Transfer failed");
      }
