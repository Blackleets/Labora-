...
const defaultQuarter = `${currentQuarter}T ${currentYear}`;
    ...
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      <section className="rounded-3xl border border-[#345947] bg-[#213B2F] p-6 text-white shadow-sm">
        <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-3xl text-sm leading-relaxed text-[#D3E3D8]">Labora+ organiza datos y estimaciones de trabajo. Nunca marca una declaración como presentada sin prueba externa verificada.</p>
          </div>
      ...
      <section className="rounded-3xl border border-[#E8DFC8] bg-[#FCFAF7] p-5 shadow-sm sm:p-6">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div><h3 className="font-serif text-base font-bold text-stone-900">Periodo de trabajo</h3><p className="mt-1 text-xs text-stone-500">Solo se usan movimientos cuya fecha cae dentro del trimestre.</p></div>
          <div className="flex flex-wrap gap-2">{[1, 2, 3, 4].map((quarterNumber) => { const quarter = `${quarterNumber}T ${currentYear}`; return <button key={quarter} onClick={() => setSelectedQuarter(quarter)} className={`rounded-xl px-3 py-1.5 text-xs font-semibold ${selectedQuarter === quarter ? 'bg-[#2E5A44] text-white' : 'border border-[#DFD5C6] bg-white text-stone-600'}`}>{quarter}</button>; })}</div>
        </div>
        <div className="mt-5 grid gap-3 md:grid-cols-4"><Metric label="Ingresos registrados" value={money(snapshot.quarter.grossIncome)} /><Metric label="Gastos aprobados deducibles" value={money(snapshot.quarter.approvedDeductibleExpenses)} /><Metric label="Gastos pendientes" value={String(snapshot.quarter.pendingExpenseCount)} emphasis={snapshot.quarter.pendingExpenseCount > 0} /><Metric label="Gastos rechazados" value={String(snapshot.quarter.rejectedExpenseCount)} /></div>
      </section>

      <div className="grid gap-4 lg:grid-cols-2">
        <section className="rounded-3xl border border-[#E8DFC8] bg-[#FCFAF7] p-5 shadow-sm">
          <div className="flex items-center justify-between"><div className="flex items-center gap-2"><Scale className="h-5 w-5 text-[#2E5A44]" /><h3 className="font-serif font-bold">Modelo 130 · estimación de trabajo</h3></div><span className="rounded-full bg-amber-50 px-2.5 py-1 text-[11px] font-bold text-amber-700">REQUIERE REVISIÓN</span></div>
          <div className="mt-5 rounded-2xl bg-[#F2EDE4] p-4"><p className="text-xs text-stone-500">Acumulado provisional antes de ajustes/pagos anteriores</p><p className="mt-1 font-serif text-3xl font-bold text-stone-900">{money(snapshot.model130.provisionalAccruedAmount)}</p></div>
          <dl className="mt-4 space-y-2 text-sm"><Line label="Ingresos acumulados" value={money(snapshot.yearToDate.grossIncome)} /><Line label="Gastos aprobados acumulados" value={money(snapshot.yearToDate.approvedDeductibleExpenses)} /><Line label="Retenciones registradas" value={money(snapshot.yearToDate.retentionsRecorded)} /></dl>
        ...
        <section className="rounded-3xl border border-[#E8DFC8] bg-[#FCFAF7] p-5 shadow-sm">
          <div className="flex items-center justify-between"><div className="flex items-center gap-2"><Scale className="h-5 w-5 text-[#3A7596]" /><h3 className="font-serif font-bold">Modelo 303 · control de IVA</h3></div><span className="rounded-full bg-sky-50 px-2.5 py-1 text-[11px] font-bold text-sky-700">DATOS INCOMPLETOS</span></div>
          <div className="mt-5 rounded-2xl bg-[#EEF4F7] p-4"><p className="text-xs text-stone-500">IVA soportado aprobado registrado</p><p className="mt-1 font-serif text-3xl font-bold text-stone-900">{money(snapshot.model303.deductibleInputVatRecorded)}</p></div>
          <p className="mt-4 text-sm leading-relaxed text-stone-600">{snapshot.model303.explanation}</p>
          <p className="mt-4 flex gap-2 rounded-2xl border border-sky-200 bg-sky-50 p-3 text-xs leading-relaxed text-sky-800"><AlertTriangle className="mt-0.5 h-4 w-4 shrink-0" />Labora+ no mostrará una deuda final de IVA hasta guardar bases y cuotas verificadas suficientes.</p>
        </section>
      </div>
