...
const ClientIdentity = ({ client, active = false, large = false }: { client: User; active?: boolean; large?: boolean }) => {
  ...
  if (image) return <img src={image} alt={client.name} className={`${size} shrink-0 rounded-full border border-[var(--labora-border)] object-cover`} />;
  return (
    <div className={`flex ${size} shrink-0 items-center justify-center rounded-full text-xs font-bold ${active ? 'bg-[var(--labora-primary)] text-white' : 'bg-[var(--labora-surface-2)] text-[var(--labora-muted)]'}`}>
      ...
      {clients.length === 0 ? (
        ...
        <section className="grid gap-5 lg:grid-cols-[300px_minmax(0,1fr)]">
          <aside className="rounded-2xl border border-[var(--labora-border)] bg-[var(--labora-surface)] p-3">
            <div className="relative mb-3"><Search size={15} className="absolute left-3 top-1/2 -translate-y-1/2 text-stone-400" /><input value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} placeholder="Buscar cliente" className="w-full rounded-xl border border-[var(--labora-border)] bg-[var(--labora-surface-2)] py-2.5 pl-9 pr-3 text-xs outline-none focus:border-[#9BB3A4]" /></div>
            <div className="space-y-1.5">
              {filteredClients.map((client) => {
                const active = selectedClient?.id === client.id;
                return <button key={client.id} onClick={() => setSelectedId(client.id)} className={`flex w-full items-center gap-3 rounded-xl p-3 text-left transition-colors ${active ? 'bg-[var(--labora-moss-soft)]' : 'hover:bg-[var(--labora-surface-2)]'}`}><ClientIdentity client={client} active={active} /><div className="min-w-0"><p className="truncate text-xs font-bold text-stone-900">{client.name}</p><p className="truncate text-[10px] text-stone-500">{client.nif || client.email}</p></div></button>;
              })}
...
const InfoCard = ({ label, value, icon: Icon }: { label: string; value: string; icon: React.ComponentType<{ size?: number; className?: string }> }) => <div className="rounded-xl bg-[var(--labora-surface-2)] p-3"><Icon size={14} className="text-stone-400" /><p className="mt-2 text-[10px] font-semibold uppercase tracking-wide text-stone-400">{label}</p><p className="mt-1 truncate text-sm font-bold text-stone-900">{value}</p></div>;
...
const ContactRow = ({ icon: Icon, label, value, href }: { icon: React.ComponentType<{ size?: number; className?: string }>; label: string; value: string; href?: string }) => {
  const content = <div className="flex items-center gap-3 rounded-xl bg-[var(--labora-surface-2)] p-3"><div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-[var(--labora-surface)] text-[var(--labora-muted)]"><Icon size={15} /></div><div className="min-w-0"><p className="text-[10px] font-semibold uppercase tracking-wide text-stone-400">{label}</p><p className="mt-0.5 truncate text-xs font-semibold text-stone-700">{value}</p></div></div>;
