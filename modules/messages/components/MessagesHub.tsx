...
import { Building2, Clock, MessageSquare, Search, Send, UserRound } from 'lucide-react';
...
export const MessagesHub: React.FC = () => {
  ...
  const contacts = useMemo(() => {
    ...
    const manager = users.find((user) => user.role === UserRole.MANAGER && user.id === currentUser.managerId);
    return manager ? [manager] : [];
  }, [currentUser, users, isManager]);

  useEffect(() => setMessages(messageRepository.getAll()), []);
  useEffect(() => {
  ...
  const handleSend = (event: React.FormEvent) => {
    ...
    const message = messageRepository.sendMessage({ personId: selectedContact.id, personName: selectedContact.name, senderId: currentUser.id, senderName: currentUser.name, recipientId: selectedContact.id, recipientName: selectedContact.name, message: draft.trim(), type: 'internal' });
      ...
      <section className="grid min-h-[520px] overflow-hidden rounded-2xl border border-[#E3DCD2] bg-white lg:grid-cols-[260px_minmax(0,1fr)]">
        ...
        <div className="flex min-h-[420px] min-w-0 flex-col">
          ...
          <div className="flex-1 space-y-3 overflow-y-auto bg-[#FAF8F4] p-4 sm:p-5">
            {!selectedContact ? <Empty icon="select" /> : conversation.length === 0 ? <Empty icon="start" /> : conversation.map((message) => {
          ...
          <form onSubmit={handleSend} className="flex gap-2 border-t border-[#ECE5DB] bg-white p-3 sm:p-4"><textarea value={draft} onChange={(e) => setDraft(e.target.value)} placeholder={selectedContact ? 'Escribe un mensaje…' : 'Sin contacto vinculado'} disabled={!selectedContact} rows={2} className="min-w-0 flex-1 resize-none rounded-xl border border-[#DED7CC] px-3 py-2.5 text-sm outline-none focus:border-[#8FA697] disabled:bg-[#F4F1EC]" /><button disabled={!selectedContact || !draft.trim()} className="self-end rounded-xl bg-[#2E5A44] p-3 text-white disabled:opacity-40" aria-label="Enviar mensaje"><Send size={17} /></button></form>
        </div>
      </section>
    </div>
  );
