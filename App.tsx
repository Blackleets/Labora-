...
const MainLayout: React.FC = () => {
  ...
  const pendingReqCount = requirements.filter((requirement) => requirement.status === 'pending' && (currentUser.role !== UserRole.RIDER || requirement.riderId === currentUser.id)).length;

  const viewTitle = (() => {
    switch (currentView) {
      case 'dashboard': return isManager ? 'Panel de asesor' : 'Inicio';
      ...
      case 'settings': return 'Perfil y seguridad';
  ...
  const renderView = () => {
    switch (currentView) {
      ...
      case 'people': return isManager ? <PeopleHub /> : <Dashboard setView={setView} />;
      ...
      <main className="relative flex h-full flex-1 flex-col overflow-hidden" style={{ backgroundColor: palette.canvas }}>
        ...
        <header className="relative z-20 flex shrink-0 items-center justify-between border-b px-4 py-3 md:px-8" style={{ backgroundColor: palette.card, borderColor: palette.border }}>
          <div className="flex items-center gap-3">
            <button onClick={() => setIsMobileMenuOpen(true)} className="rounded-xl p-2 text-stone-600 hover:bg-[#F2EDE4] lg:hidden" aria-label="Abrir menú"><Menu size={20} /></button>
            <div className="hidden items-center gap-2 sm:flex"><span className="text-xs font-bold uppercase tracking-wider text-stone-400">Labora+</span><span className="text-stone-300">/</span><span className="font-serif text-sm font-bold text-stone-900">{viewTitle}</span></div>
            <div className="sm:hidden"><Logo size="sm" showText={false} /></div>
          </div>

          <div className="flex items-center gap-2">
            <GhibliLightingControl />
            <button onClick={togglePrivacyMode} className={`rounded-xl border p-2 ${privacyMode ? 'border-[#FDE3B8] bg-[#FEF7EB] text-[#85531B]' : 'border-[#EAE3D6] bg-[#FCFAF7] text-stone-500'}`} title={privacyMode ? 'Mostrar importes' : 'Ocultar importes'}>{privacyMode ? <EyeOff size={16} /> : <Eye size={16} />}</button>
            {!isManager ? (
              <button onClick={() => setIsGlobalGasModalOpen(true)} className="flex items-center gap-1.5 rounded-xl bg-[#D96B43] px-3 py-1.5 text-xs font-semibold text-white shadow-sm"><Fuel size={14} /><span className="hidden sm:inline">Guardar ticket</span><span className="sm:hidden">Ticket</span></button>
            ) : (
              <button onClick={() => setView('gestor-requirements')} className="flex items-center gap-1.5 rounded-xl bg-[#2E5A44] px-3 py-1.5 text-xs font-semibold text-white shadow-sm"><Plus size={14} /><span className="hidden sm:inline">Pedir documento</span><span className="sm:hidden">Pedir</span></button>
            )}
            <button onClick={() => setView('gestor-requirements')} className="relative rounded-xl border border-[#EAE3D6] bg-[#FCFAF7] p-2 text-stone-600" title="Avisos"><Bell size={16} />{pendingReqCount > 0 && <span className="absolute -right-1 -top-1 flex h-4 w-4 items-center justify-center rounded-full bg-[#D9943B] text-[9px] font-black text-white">{pendingReqCount}</span>}</button>
            <button onClick={() => setView('settings')} className={`flex h-8 w-8 items-center justify-center rounded-xl text-white shadow-sm ${isManager ? 'bg-[#3A7596]' : 'bg-[#2E5A44]'}`} title="Perfil autenticado">{isManager ? <Briefcase size={14} /> : <Bike size={14} />}</button>
          </div>
        </header>

        <div className="relative z-10 flex-1 overflow-y-auto p-4 md:p-8"><div className="mx-auto min-h-full max-w-7xl">{renderView()}</div></div>
        ...
        <nav className="z-30 flex shrink-0 items-center justify-around border-t border-stone-200 bg-[#FCFAF7] px-3 py-2.5 lg:hidden">
          {navItems.map((item) => {
            ...
            return <button key={item.id} onClick={() => setView(item.id)} className={`relative flex flex-col items-center gap-1 rounded-lg px-2 py-1 ${isActive ? 'text-[#2E5A44]' : 'text-stone-400'}`}><div className="relative"><Icon size={19} strokeWidth={isActive ? 2.5 : 2} />{Boolean(item.badge && item.badge > 0) && <span className="absolute -right-2 -top-1 h-3.5 min-w-3.5 rounded-full bg-[#D9943B] px-0.5 text-[8px] font-black text-white">{item.badge}</span>}</div><span className="text-[10px] font-bold">{item.label}</span></button>;
          })}
        </nav>
      </main>
